...
#define bsg_dram_uint8_store(dram_addr,val) do { *(bsg_dram_uint8_ptr((dram_addr))) = (unsigned char) (val); } while (0)
#define bsg_dram_uint8_load(dram_addr,val)  do { val = *(bsg_dram_uint8_ptr((dram_addr))) ; } while (0)
